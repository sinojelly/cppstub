...
union _THorribleUnion{
	OutputClass out;
	InputClass in;
};

template <class OutputClass, class InputClass>
inline OutputClass MyHorribleCast(const InputClass input){
	_THorribleUnion<OutputClass, InputClass> u;
...
inline DWORD STUB_MethodAddr(MethodPointer p)
{
	DWORD dwAddr;
	char szBuf[20];
	sprintf(szBuf, "%d", MyHorribleCast<DWORD>(p)); // CSimple::SimpleMethod
